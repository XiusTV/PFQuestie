local questAnnounceFrame = CreateFrame("Frame", "pfQuestbronzebeardAnnounce")
...
questAnnounceFrame:SetScript("OnEvent", function(self, event, message)
  if event == "UI_INFO_MESSAGE" and message then
    pfQuestbronzebeard_OnQuestUpdate(message)
  end
end)

function pfQuestbronzebeard_OnQuestUpdate(message)
  ...
  if itemName and numItems and numNeeded then
    ...
    local questName = pfQuestbronzebeard_GetQuestNameForObjective(itemName)
    local outMessage

    if stillNeeded < 1 then
      if pfQuest_config["bronzebeardannounceFinished"] == "1" then
        if questName then
      ...
      if pfQuest_config["bronzebeardannounceRemaining"] == "1" then
        if questName then
...
function pfQuestbronzebeard_GetQuestNameForObjective(objectiveName)
...
local function ExtendPfQuestConfig()
  ...
  for _, entry in pairs(pfQuest_defconfig) do
    if entry.text == "Announce" and entry.type == "header" then
      foundHeader = true
    elseif entry.config == "bronzebeardannounceFinished" then
      foundFinished = true
    elseif entry.config == "bronzebeardannounceRemaining" then
      foundRemaining = true
    end
  end
  ...
  if not foundFinished then
    table.insert(pfQuest_defconfig, {
      ...
      default = "1",
      type = "checkbox",
      config = "bronzebeardannounceFinished"
    })
  end
  ...
  if not foundRemaining then
    table.insert(pfQuest_defconfig, {
      ...
      default = "0",
      type = "checkbox",
      config = "bronzebeardannounceRemaining"
    })
  end

  if not pfQuest_config["bronzebeardannounceFinished"] then
    pfQuest_config["bronzebeardannounceFinished"] = "1"
  end
  if not pfQuest_config["bronzebeardannounceRemaining"] then
    pfQuest_config["bronzebeardannounceRemaining"] = "0"
  end
...
configExtenderFrame:SetScript("OnEvent", function(self, event, addonName)
  if addonName == "pfQuest-bronzebeard" then
    local timer = 0
    self:SetScript("OnUpdate", function()
      ...
      if timer > 10 then
        ...
        elseif timer > 300 then
          self:SetScript("OnUpdate", nil)
          self:UnregisterAllEvents()
          DEFAULT_CHAT_FRAME:AddMessage("|cff33ffccpf|cffffffffQuest |cffcccccc[bronzebeard]|r: Config integration failed")
        end
      end
    end)
  end
  ...
  SlashCmdList["PFDB"] = function(input, editbox)
    ...
    if arg1 == "announce" then
      if arg2 == "finished" then
        if arg3 == "on" then
          pfQuest_config["bronzebeardannounceFinished"] = "1"
          DEFAULT_CHAT_FRAME:AddMessage("|cff33ffccpf|cffffffffQuest |cffcccccc[bronzebeard]|r: Finished quest announcements enabled")
          return
        elseif arg3 == "off" then
          pfQuest_config["bronzebeardannounceFinished"] = "0"
          DEFAULT_CHAT_FRAME:AddMessage("|cff33ffccpf|cffffffffQuest |cffcccccc[bronzebeard]|r: Finished quest announcements disabled")
          return
        end
      elseif arg2 == "remaining" then
        if arg3 == "on" then
          pfQuest_config["bronzebeardannounceRemaining"] = "1"
          DEFAULT_CHAT_FRAME:AddMessage("|cff33ffccpf|cffffffffQuest |cffcccccc[bronzebeard]|r: Remaining quest announcements enabled")
          return
        elseif arg3 == "off" then
          pfQuest_config["bronzebeardannounceRemaining"] = "0"
          DEFAULT_CHAT_FRAME:AddMessage("|cff33ffccpf|cffffffffQuest |cffcccccc[bronzebeard]|r: Remaining quest announcements disabled")
          return
        end
      end
    end
