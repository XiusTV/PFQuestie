...
function CheckObjectiveProgress()
  ...
  -- OPTIMIZATION: Throttle to max once per 0.5 seconds
  local now = GetTime()
...
-- OPTIMIZED: Periodic scan and inject timer (increased from 3 to 5 minutes to reduce overhead)
local periodicScanFrame = CreateFrame("Frame")
periodicScanFrame.timer = 0
periodicScanFrame.scanInterval = 300 -- 5 minutes in seconds (was 180)
periodicScanFrame:SetScript("OnUpdate", function()
  ...
  if this.timer >= this.scanInterval then
    ...
    -- Auto-inject all captured quests into live database (silent, no debug spam)
...
capture:RegisterEvent("PLAYER_TARGET_CHANGED") -- Cache NPC when you target them
capture:RegisterEvent("UPDATE_MOUSEOVER_UNIT") -- Cache NPC when you mouseover them
capture:RegisterEvent("GOSSIP_SHOW") -- Capture NPC when gossip menu opens
capture:RegisterEvent("QUEST_GREETING") -- Capture NPC when quest greeting shows
...
capture:SetScript("OnEvent", function()
  ...
  if event == "PLAYER_LOGIN" then
    -- Merge injected data from SavedVariables into pfDB
    ...
    deferFrame:SetScript("OnUpdate", function()
      ...
      for title, data in pairs(captureData) do
        if data.level then
          ...
          -- Inject into live pfDB database (with message if debug enabled)
          InjectQuestIntoDatabase(title, data, pfQuest_CaptureConfig.debug)
        end
      end
      
      -- Also inject all other captured quests (silent)
  ...
  elseif event == "QUEST_ACCEPTED" then
    ...
    pfQuestCapture_DelayFrame:SetScript("OnUpdate", function()
      ...
      if this.timer >= 0.5 then
        this:SetScript("OnUpdate", nil)
        ScanQuestLog(false) -- Silent scan
        CheckObjectiveProgress() -- Check for objective changes
        
        -- OPTIMIZATION: Only inject on quest acceptance, not every log update
        InjectAllCapturedQuests(false) -- Silent injection
      end
    end)
  elseif event == "QUEST_LOG_UPDATE" then
    -- OPTIMIZATION: Only check objectives on log update, don't inject or scan
    -- This event fires VERY frequently and was causing stuttering
    CheckObjectiveProgress() -- Check for objective changes (already throttled)
  elseif event == "PLAYER_LOGIN" then
    ...
    delayFrame:SetScript("OnUpdate", function()
      ...
      if this.timer >= 2.0 then
        ...
        -- Auto-inject all captured quests on login (silent unless debug)
