...
local major, minor, fix = hcstrsplit(".", tostring(GetAddOnMetadata("pfQuest-bronzebeard", "Version")))
        ...
        if arg1 == "pfqb" then
            ...
            if v == "VERSION" and remoteversion then
                if remoteversion > localversion then
                    ...
                    if not alreadyshown then
                        print("|cff33ffccpf|cffffffffQuest |cffcccccc[Bronzebeard DB] |cffff5555[RELEASE]|r New version available! |cff66ccffhttps://github.com/Bennylavaa/pfQuest-bronzebeard|r")
                        alreadyshown = true
                    end
                end
            end
            ...
            if v == "PING?" then
                for _, chan in ipairs(loginchannels) do
                    SendAddonMessage("pfqb", "PONG!:"..GetAddOnMetadata("pfQuest-bronzebeard", "Version"), chan)
                end
            end
            if v == "PONG!" then
        ...
        if event == "PARTY_MEMBERS_CHANGED" then
            ...
            if (this.group or 0) < groupsize then
                for _, chan in ipairs(groupchannels) do
                    SendAddonMessage("pfqb", "VERSION:" .. localversion, chan)
                end
            end
