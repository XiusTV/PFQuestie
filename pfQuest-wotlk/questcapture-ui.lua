...
-- Fallback values if pfUI isn't loaded
...
local function CreateBackdrop(frame, inset, border, alpha)
  if pfUI and pfUI.api and pfUI.api.CreateBackdrop then
    pfUI.api.CreateBackdrop(frame, inset, border, alpha)
  else
    -- Fallback: simple backdrop
    frame:SetBackdrop({
    ...
    frame:SetBackdropColor(0.1, 0.1, 0.1, alpha or 0.85)
    frame:SetBackdropBorderColor(0.4, 0.4, 0.4, 1)
  end
end

local function SkinButton(button, r, g, b)
  if pfUI and pfUI.api and pfUI.api.SkinButton then
    pfUI.api.SkinButton(button, r, g, b)
  end
end

-- Wait for pfUI to load before creating UI
local captureUILoader = CreateFrame("Frame")
captureUILoader:RegisterEvent("ADDON_LOADED")
captureUILoader:SetScript("OnEvent", function()
  if arg1 == "pfQuest" or arg1 == "pfQuest-tbc" or arg1 == "pfQuest-wotlk" then
    -- pfUI should be loaded by now, create the UI
    CreateCaptureUI()
...
function CreateCaptureUI()
  if pfQuestCaptureUI then return end -- Already created
  ...
  CreateBackdrop(pfQuestCaptureUI, nil, true, 0.85)
  ...
  pfQuestCaptureUI.title:SetFont(GetFont(), GetFontSize() + 1, "OUTLINE")
  ...
  SkinButton(pfQuestCaptureUI.closeBtn, 1, 0.3, 0.3)
...
pfQuestCaptureUI.resizegrip:SetScript("OnLeave", function()
  ...
  pfQuestCaptureUI.statusBar:SetFont(GetFont(), GetFontSize() - 1, "OUTLINE")
  pfQuestCaptureUI.statusBar:SetJustifyH("LEFT")

  -- Toggle capture button (styled like bottom buttons)
  pfQuestCaptureUI.toggleBtn = CreateFrame("Button", nil, pfQuestCaptureUI)
  pfQuestCaptureUI.toggleBtn:SetPoint("TOPRIGHT", pfQuestCaptureUI, "TOPRIGHT", -30, -4)
  pfQuestCaptureUI.toggleBtn:SetWidth(55)
  pfQuestCaptureUI.toggleBtn:SetHeight(26)
  pfQuestCaptureUI.toggleBtn.text = pfQuestCaptureUI.toggleBtn:CreateFontString(nil, "OVERLAY", "GameFontNormal")
  pfQuestCaptureUI.toggleBtn.text:SetPoint("CENTER")
  pfQuestCaptureUI.toggleBtn.text:SetFont(GetFont(), GetFontSize(), "OUTLINE")
  ...
  pfQuestCaptureUI.toggleBtn:SetScript("OnClick", function()
    ...
    pfQuestCaptureUI.toggleBtn.UpdateAppearance()  -- Update button appearance immediately
  ...
  pfQuestCaptureUI.toggleBtn:SetScript("OnEnter", function()
    GameTooltip:SetOwner(this, "ANCHOR_TOP")
    GameTooltip:ClearLines()
    GameTooltip:AddLine("Toggle Quest Capture", 1, 1, 1)
    if pfQuest_CaptureConfig.enabled then
      GameTooltip:AddLine("Currently: |cff55ff55ENABLED", 0.7, 0.7, 0.7)
      GameTooltip:AddLine("Click to disable", 0.7, 0.7, 0.7)
    else
      GameTooltip:AddLine("Currently: |cffff5555DISABLED", 0.7, 0.7, 0.7)
      GameTooltip:AddLine("Click to enable", 0.7, 0.7, 0.7)
    end
    GameTooltip:Show()
  end)
  
  pfQuestCaptureUI.toggleBtn:SetScript("OnLeave", function()
    GameTooltip:Hide()
  end)
...
local function GetCaptureEntry()
  ...
  entry:SetHeight(40)
  CreateBackdrop(entry, nil, true, 0.4)
  ...
  entry.title:SetFont(GetFont(), GetFontSize() - 1, "OUTLINE")
  ...
  entry.info:SetFont(GetFont(), GetFontSize() - 2, "OUTLINE")
...
function pfQuestCaptureUI:UpdateUI()
  ...
  if self.toggleBtn and self.toggleBtn.UpdateAppearance then
    self.toggleBtn.UpdateAppearance()
  end
...
end -- End of CreateCaptureUI() function
